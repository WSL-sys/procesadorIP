...
n_paginas = X.n_paginas%File_Size / (lengthx * Ncanales * dato_length + header_length);
largo_datos = lengthx * Ncanales * dato_length;

arr(1:largo_datos)=0;
%*n_paginas
datos(lengthx*Ncanales)=0;
...
for j=1:n_paginas
        off = (j-1)*(lengthx); %ultima posicion en el arreglo.
        %calcula la fecha..
        fecha=datevec(datestr(tiempo(off+1)/3600/24 + datenum(2000,1,1,0,0,0),0)); %calcula la fecha
      ...
      %  fwrite(fid,X.datos(off+1:+offn_muestras,:),'integer*3')
        ...
        for i=1:lengthx
         ...
         fwrite(fid,datos(1:lengthx*Ncanales*3),'uint8');
    %indica el avance...
    %if cont > 10
     %   cont = 0;
        %waitbar(j/n_paginas);
...
arr(5) = header.mes;
arr(6) = header.ano;
arr(7) = 7;  %JAB 13 de noviembre para que reconosca los TS2 en SSMT2000
arr(8) = 20; %
