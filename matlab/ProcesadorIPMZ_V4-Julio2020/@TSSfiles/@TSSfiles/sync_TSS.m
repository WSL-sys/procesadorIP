function res = sync_TSS(file,destino,inicio,final)
%X=sync_TSS(file)
%Sincroniza un fichero TSS y lo divide en archivos por canales
res=0;
X = TSSfiles;
%contantes.
...
fid = ffopen(file);
%lee el header
[header,off_set] = leer_header(fid,0); 
...
for j=1:n_paginas
        ...
        [header_tmp,off_set] = leer_header(fid,off_set);
        tiempo_tmp = header_tmp.dat;
        if tiempo_tmp < datenum_final
            if tiempo_tmp < datenum_inicio
            else
...
% X = crearTSS(X,file,destino);

function [header,offset]=leer_header(fid,offset)
...
function X = crearTSS(X,file,destino)
datos = X.datos;
tmp = strread(file,'%s','delimiter','\\');
path=destino;
[filename,ext] = strtok(char(tmp{end}),'.');
for i=1:length(datos(1,:))
    filen = [path,filename,'_CH',i,ext];
    write_TSS(X,filen);
end
